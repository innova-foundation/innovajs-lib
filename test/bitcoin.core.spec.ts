...
describe('Bitcoin-core', () => {
  ...
  // base58KeysValid
  describe('address.toBase58DCheck', () => {
    const typeMap: any = {
      pubkey: 'pubKeyHash',
      script: 'scriptHash',
    };
    ...
    base58KeysValid.forEach(f => {
      ...
      it('can export ' + expected, () => {
        assert.strictEqual(
          bitcoin.address.toBase58DCheck(hash, version),
          expected,
        );
      });
    });
  ...
  // base58KeysInvalid
  describe('address.fromBase58DCheck', () => {
    ...
    base58KeysInvalid.forEach(f => {
      ...
      it('throws on ' + strng, () => {
        assert.throws(() => {
          const address = bitcoin.address.fromBase58DCheck(strng);
